...
def room_vertices_segment(house_facet,house_vertices,house_norm_vector):
    ...
    xyz_minmax=np.array([-100,3200,-100,1600,-100,3000])*0.001
    room_facet=[]
    room_norm_vector=[]
    room_vertices = []
    j=0
...
def room_renovation_plane_vertices_selection(room_facet, room_norm_vector, room_vertices):
    m=1
    vertices_flag=np.zeros(len(room_norm_vector))
    for i in range(len(room_norm_vector)):
        if abs(room_norm_vector[i,0])<=0.01 and abs(room_norm_vector[i,1])<=0.01 and abs(room_norm_vector[i,2])<=1.01:
            vertices_flag[i]=1
    for i in range(len(room_norm_vector)):
        if abs(room_vertices[3*i,0])<=0.1 and abs(room_vertices[3*i+1,0])<=0.1 and abs(room_vertices[3*i+2,0])<=0.1:
            vertices_flag[i]=1
        if abs(room_vertices[3*i,1]-1.50)<=0.1 and abs(room_vertices[3*i+1,1]-1.50)<=0.1 and abs(room_vertices[3*i+2,1]-1.50)<=0.1:
            vertices_flag[i]=1
    k=0
...
def boundary_cell_generation(plane_boundaries):
    ...
    while(1):
        ...
        if (flag1==np.zeros(boundary_num)).all():
            break
        if (flag2==np.ones(boundary_cell_num)):
            boundary_cell_num+=1
...
def room_planes_paths_generation(room_plane_edge_cell,room_plane_norm_vector):
    ...
    return room_plane_intersect_point
    ## return room_planes_boundary_cell_sorted
...
def visualization_cube(num, position, color, scale):
    ...
    return marker
def rivz_visualization_room_renovation_waypoints(room_plane_intersect_point):
    ...
    num = 1
    for i in range(len(room_plane_intersect_point)):
        for j in range(len(room_plane_intersect_point[i])):
            intersect_point = room_plane_intersect_point[i][j]
            position.x = intersect_point[0]
            position.y = intersect_point[1]
            position.z = intersect_point[2]
...
def rviz_visualization_room(room_facet,room_norm_vector,room_vertices):
    # in the first step, vertices, norm_vector and facet should be tranformed into a new stl docement named as "room.stl"
    ...
    # in the second step, the stl document should be visualized
    str1='stl_document/room_0015.STL'
    rviz_visualization_stl(str1)

# def rviz_visualization_house():
...
if __name__ == "__main__":
    house_facet,house_norm_vector,house_vertices,house_vector=house_stl_reading("stl_document/0015.STL")
    # house_stl_matplot(house_vertices)
    ...
    room_surface_division_planes(room_plane_edge_cell, room_plane_norm_vector)


    # rviz_visualization_stl()
    # rviz_visualization_room(room_facet_1,room_norm_vector_1,house_vertices)
    # str='zy'
    # rviz_visualization_stl(str)
    # your_mesh = mesh.Mesh.from_file('stl_document/room_0015.STL')
    # print(your_mesh.v0)
